...
from logic_program_interface import Action, Direction, LogicProgramInterface, Solution
from sokoban_map import SokobanMap, Tile
    ...
    def set_goal(self, steps: int) -> LogicProgramOptimized:
        ...
        for x in range(self._sokoban_map.height):
            for y in range(self._sokoban_map.width):
                if self._sokoban_map.grid[x][y] == Tile.STORAGE:
    ...
    def __add_init(self) -> None:
        ...
        for x in range(self._sokoban_map.height):
            for y in range(self._sokoban_map.width):
                ...
                if (x, y) in self._sokoban_map.crates:
        ...
        for x in range(self._sokoban_map.height):
            ...
            if x == self._sokoban_map.sokoban[0]:
                self._init_state.append(sx)
            else:
                self._init_state.append(-sx)
        for y in range(self._sokoban_map.width):
            sy = self.__sokobanY_var(y, 0)
            if y == self._sokoban_map.sokoban[1]:
